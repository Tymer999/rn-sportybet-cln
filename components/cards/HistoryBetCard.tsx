...
const HistoryBetCard: FC<HistoryBetCardProps> = ({
  bet,
  showDate,
  isFirst,
}) => {
  ...
  const [wonBet, setWonBet] = useState(bet || null);
  ...
  return (
    <>
      <TouchableWithoutFeedback onPress={handlePress}>
        <View
          className={`flex-row mb-3 pt-3 pl-3 ${
            showDate && !isFirst && "border-t-[.3px] border-t-gray-light"
          }`}
        >
          {showDate ? (
            ...
            >
              <View className="w-[3.25rem]">
                <Text className="text-xl font-bold text-gray">
                  {day < 10 && 0}
                  {day}
                </Text>
          ...
          <View className="flex-1">
            ...
            <View className="flex-row items-end mb-3 justify-between mr-8">
              <View className="flex mr-4 gap-1 flex-1">
                {bet.matches.map(
                  (match, index) =>
                    index < 3 && (
                      <Text
                        key={index}
                        className="text-gray flex-1"
                        numberOfLines={1}
                      >
                ...
                {bet.matches.length > 3 && (
                  <Text className="text-gray">
                    ...(and {bet.matches.length - 3} other matches)
                  </Text>
                )}
